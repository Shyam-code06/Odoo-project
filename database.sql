...
CREATE TABLE `roles` (
  ...
  `code` VARCHAR(50) NULL UNIQUE COMMENT 'Role code e.g. EMPLOYEE, HR_MANAGER, ADMIN',
...
CREATE TABLE `attendance` (
  ...
  `latitude` DECIMAL(10, 7) NULL COMMENT 'GPS latitude at verification',
  `longitude` DECIMAL(10, 7) NULL COMMENT 'GPS longitude at verification',
  `verification_distance` DECIMAL(8, 2) NULL COMMENT 'Calculated distance from office in meters',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
