...
// Structured Mock Database Datasets
...
const MOCK_NEEDS_ATTENTION_BY_ROLE = {
  [ROLES.EMPLOYEE]: [
...
const MOCK_QUICK_ACTIONS_BY_ROLE = {
  [ROLES.EMPLOYEE]: [
...
export const dashboardService = {
  ...
  approveTimeOff: async (requestId) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        MOCK_TIME_OFF_REQUESTS = MOCK_TIME_OFF_REQUESTS.map((req) =>
          req.id === requestId ? { ...req, status: 'Approved' } : req
        );
        resolve({ success: true, requests: MOCK_TIME_OFF_REQUESTS });
      }, 200);
    });
  },
