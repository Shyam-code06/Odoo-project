...
export class PayrunService {
    ...
    for (const emp of allEmployees) {
      ...
      if (isEligible) {
        const duplicateRun = await db('payrun_employees')
          ...
          .select('payruns.id', 'payruns.name', 'payruns.status', 'payruns.period_start', 'payruns.period_end')
          .first();
  ...
  async createPayrun(userId, payload) {
    ...
    if (salary_structure_id) {
      ...
      if (!structure || !structure.is_active) {
        const error = new Error(`Salary structure with ID ${salary_structure_id} does not exist or is inactive`);
    ...
    for (const empId of employee_ids) {
      ...
      try {
        const contract = await contractService.getApplicableContract(empId, period_start, period_end);
    ...
    if (validationErrors.length > 0) {
      const error = new Error(`Cannot create payrun due to employee validation errors: ${validationErrors.join('; ')}`);
      error.statusCode = 400;
  ...
  async getPayruns(params = {}) {
    ...
    const countResult = await query.clone().count('payruns.id as total').first();
    ...
    if (payrunIds.length > 0) {
      const counts = await db('payrun_employees')
        ...
        .select(
          ...
          db.raw("SUM(CASE WHEN status = 'computed' THEN 1 ELSE 0 END) as computed_count"),
  ...
  async addEmployeesToPayrun(payrunId, employeeIds) {
    ...
    if (payrun.status !== 'draft') {
      const error = new Error(`Cannot add employees to a payrun with status '${payrun.status}' (must be 'draft')`);
      error.statusCode = 400;
  ...
  async removeEmployeeFromPayrun(payrunId, employeeId) {
    ...
    if (payrun.status !== 'draft') {
      const error = new Error(`Cannot remove employees from a payrun with status '${payrun.status}' (must be 'draft')`);
      error.statusCode = 400;
      error.code = 'INVALID_PAYRUN_STATUS';
      throw error;
    }
  ...
  async computePayrun(payrunId) {
    ...
    const assignedEmployees = await db('payrun_employees').where({ payrun_id: payrunId });
    if (assignedEmployees.length === 0) {
      const error = new Error('Cannot compute payrun with 0 assigned employees');
      error.statusCode = 400;
  ...
  async validatePayrun(payrunId) {
    ...
    const employees = await db('payrun_employees').where({ payrun_id: payrunId });
    if (employees.length === 0) {
      const error = new Error('Cannot validate payrun with 0 assigned employees');
      error.statusCode = 400;
      error.code = 'EMPTY_PAYRUN';
      throw error;
    }
    ...
    const duplicates = await db('payrun_employees')
      ...
      .select('employees.first_name', 'employees.last_name', 'employees.employee_code', 'payruns.name', 'payruns.status');

    if (duplicates.length > 0) {
      const dupNames = duplicates.map((d) => `${d.first_name} ${d.last_name} in '${d.name}' (${d.status})`).join('; ');
      const error = new Error(
