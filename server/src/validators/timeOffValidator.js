...
export const validateAllocationCreate = (req, res, next) => {
  ...
  if (
    !time_off_type_id ||
    isNaN(Number(time_off_type_id)) ||
    Number(time_off_type_id) <= 0
  ) {
    errors.push({
      field: "time_off_type_id",
      message: "Valid time off type ID is required",
    });
  }
  ...
  if (
    ...
    errors.push({
      ...
      message: "Allocated amount must be a positive number greater than 0",
    });
  }
...
export const validateLeaveRequestCreate = (req, res, next) => {
  ...
  if (employee_id !== undefined && employee_id !== null) {
    if (isNaN(Number(employee_id)) || Number(employee_id) <= 0) {
      errors.push({
        field: "employee_id",
        message: "Employee ID must be a positive integer if provided",
      });
    }
  }
...
export const validateLeaveRequestReject = (req, res, next) => {
  ...
  req.body.rejected_reason =
    typeof rejected_reason === "string" ? rejected_reason.trim() : null;
  next();
};
